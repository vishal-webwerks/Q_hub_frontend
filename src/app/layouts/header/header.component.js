"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
var core_1 = require("@angular/core");
var router_1 = require("@angular/router");
var globs = require("../../services/globals");
var auth_service_1 = require("../../services/auth.service");
var AppHeaderComponent = /** @class */ (function () {
    function AppHeaderComponent(auth, router) {
        var _this = this;
        this.auth = auth;
        this.router = router;
        this.authState = false;
        this.cur_Url = false;
        this.inactive_class = "";
        this.userInfo = {
            appEmpName: '',
            appUsername: '',
            appProffession: '',
            appUserID: ''
        };
        this.appName = globs.appName;
        this.authState = this.auth.authStatus();
        this.router.events.subscribe(function (val) {
            _this.authState = _this.auth.authStatus();
            _this.userInfo = _this.auth.getUser();
            _this.cur_Url = _this.router.url;
        });
    }
    AppHeaderComponent.prototype.toggle_sidebar = function () {
        if (this.inactive_class == "") {
            this.inactive_class = "inactive";
        }
        else {
            this.inactive_class = "";
        }
    };
    AppHeaderComponent.prototype.logout = function () {
        this.auth.logout();
    };
    AppHeaderComponent = __decorate([
        core_1.Component({
            selector: 'app-header',
            templateUrl: 'app/layouts/header/header.component.html',
            providers: [auth_service_1.AuthService]
        }),
        __metadata("design:paramtypes", [auth_service_1.AuthService, router_1.Router])
    ], AppHeaderComponent);
    return AppHeaderComponent;
}());
exports.AppHeaderComponent = AppHeaderComponent;
//# sourceMappingURL=header.component.js.map